...
export default async function DashboardPage() {
  ...
  try {
    const latestByCategoryResult = await sql`SELECT DISTINCT ON (signal_category) signal_category, signal_name, reading_value, reading_text, threshold_value, threshold_breached, reading_date FROM signal_readings ORDER BY signal_category, reading_date DESC, created_at DESC`;
    ...
    const clearedResult = await sql`SELECT COUNT(*)::int AS count FROM signal_readings WHERE threshold_breached = FALSE AND reading_date >= NOW() - INTERVAL '14 days'`;
  ...
  return <main>
    ...
    <table><thead><tr><th>Category</th><th>Signal</th><th>Last Reading</th><th>Threshold</th><th>Breach</th><th>Date</th></tr></thead><tbody>
      {latestByCategory.map((r) => <tr key={`${r.signal_category}-${r.signal_name}`}><td>{r.signal_category}</td><td>{r.signal_name}</td><td>{r.reading_value ?? r.reading_text ?? 'N/A'}</td><td>{r.threshold_value ?? 'N/A'}</td><td>{r.threshold_breached ? 'Yes' : 'No'}</td><td>{String(r.reading_date).slice(0,10)}</td></tr>)}
    </tbody></table>
    <h2>Active vs cleared breaches (last 14 days)</h2>
    <p>Active: {activeCount} | Cleared: {clearedCount}</p>
