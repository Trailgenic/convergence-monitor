...
export async function GET() {
  ...
  return NextResponse.json({ readings: readings.rows, status: aggregateConvergence(latestRegistryRows as AggregationReading[]).status, events: events.rows, lifecycleEvents: lifecycleEvents.rows });
}
